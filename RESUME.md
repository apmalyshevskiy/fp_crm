...
1. Клиент / Телефон
2. Название компании (для КП) / ИНН (для КП) — у ИНН кнопка «🔍 Найти» (поиск контрагента через DaData)
...
5. ✅ Поиск контрагента по ИНН через DaData (кнопка «🔍 Найти» у поля ИНН; токен хранится в `localStorage`, ключ `fpos_dadata_token`; бесплатно ~10k запросов/сут). Вариант «открытые данные ФНС» рассмотрен и отклонён: бесплатного официального API «ИНН→название» нет (только платная подписка на XML-дамп ЕГРЮЛ или сторонние API типа egrul.org с лимитом 100/сут)
...
- [ ] DaData-токен лежит в `localStorage` открыто — при необходимости спрятать через прокси на бэкенде (свой PHP-эндпоинт, ключ на сервере)
